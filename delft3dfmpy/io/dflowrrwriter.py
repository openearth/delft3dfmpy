...
class DFlowRRWriter:
    ...
    def __init__(self, rrmodel, output_dir, name):
        ...
        self.src_dir = os.path.join(os.path.dirname(__file__), 'resources', 'RRmodel')
    ...
    def write_topology(self):
        ...
        # nwrw
        # NODE id ?0-28? ri ?1? mt 1 ?7? nt 3 ObID ?SBK_CONN&RUNOFF? px 198002.5 py 457582.5 node
        filepath = os.path.join(self.output_dir, '3BRUNOFF.TP')
        with open(filepath, 'w') as f:
            if any(self.rrmodel.nwrw.nwrw_nodes):
                for _, dct in self.rrmodel.nwrw.nwrw_nodes.items():
                    if float(sum(dct['ar'])) > 0.0:
                        f.write(f"NODE id '{dct['id']}' ri '-1' mt 1 '7' nt 0 ObID 'SBK_CONN&LAT&RUNOFF' px {dct['px']:.2f} py {dct['py']:.2f} node\n")
    ...
    def write_nwrw(self):
         """
         Method to write pluvis.3b file.
        ...
        """
         # write pluvis.3b
         if any(self.rrmodel.nwrw.nwrw_nodes):
             filepath = os.path.join(self.output_dir, 'pluvius.3b')
             with open(filepath, 'w') as f:
                 for _, dct in self.rrmodel.nwrw.nwrw_nodes.items():
                     f.write('NWRW' +
                             ' id \''+dct['id'] + '\'' +
                             ' ar '+' '.join([str(a) for a in dct['ar']])+
                             ' np '+ str(dct['np']) +
                             ' dw \''+dct['dw'] + '\'' +
                             ' np2 0' +
                             ' dw2 \''+dct['dw'] + '\'' +
    ...
    def write_meteo(self):
        ...
        """
         # precip
        if len(self.rrmodel.external_forcings.precip) > 0:
            df = {}
...
#%%%
        # evaporation
        if len(self.rrmodel.external_forcings.evap)>0:
    ...
    def read_config(self):
        ...
        """
        read configurations (DELFT_3B.INI)
        """
        ...
        _ = config.read(os.path.join(self.src_dir,'DELFT_3B.INI'))

        self.config = config

    def update_config(self):
        """
        update rr configurations (DELFT_3B.INI) based on dict

        # FIXME BMA: hydromt related format
        """

        if self.config is None:
            self.read_config()

        if self.d3b_parameters is not None:
            for sec in self.d3b_parameters.keys():
                for k,v in self.d3b_parameters[sec].items():
                    if v is not None:
                        self.config.set(sec, k, str(v))

        with open(os.path.join(self.output_dir,'DELFT_3B.INI'), 'w') as configfile:
    ...
    def write_dimr(self, DFM_comp_name= 'DFLOWFM', RR_comp_name = 'DRR'):
        """
        Method to write the files required for coupling DFM and RR via nwrw.
        """
        ...
        with open(os.path.join(self.output_dir, '../dimr_config.xml'), 'w') as f:
            ...
            f.write('\t<documentation>\n')
            f.write('\t\t<fileVersion>1.2</fileVersion>\n')
            f.write('\t\t<createdBy>delft3dfmpy-bangkok</createdBy>\n')
            f.write("\t\t<creationDate>" + datetime.datetime.now().isoformat() + '</creationDate>\n')
            f.write('\t</documentation>\n\n')
            f.write('\t<control>\n')
            f.write('\t\t<parallel>\n')
            f.write('\t\t\t<startGroup>\n')
            # change here!!!
            tstr = f'0 {float(self.config["TimeSettings"]["TimestepSize"]):.0f} {(pd.Timestamp(self.config["TimeSettings"]["EndTime"]) - pd.Timestamp(self.config["TimeSettings"]["StartTime"])).total_seconds():.0f}'
            f.write('\t\t\t\t<time>' + tstr + '</time>\n')
            f.write('\t\t\t\t<start name="' + RR_comp_name + '" />\n')
            f.write('\t\t\t\t<coupler name="rr2flow" />\n')
            f.write('\t\t\t</startGroup>\n')
            ...
            f.write('\t<component name="' + DFM_comp_name + '">\n')
            f.write('\t\t<library>dflowfm</library>\n')
            f.write('\t\t<workingDir>dflowfm</workingDir>\n')
            f.write('\t\t<inputFile>' + DFM_comp_name + '.mdu</inputFile>\n')
            f.write('\t</component>\n')
            f.write('\t<component name="' + RR_comp_name + '">\n')
            f.write('\t\t<library>rr_dll</library>\n')
            f.write('\t\t<workingDir>rr</workingDir>\n')
            ...
            for i in self.rrmodel.nwrw.nwrw_nodes.items():
                f.write('\t\t\t<item>\n')
                f.write('\t\t\t\t<sourceName>catchments/' + str(i[0]) + '/water_discharge</sourceName>\n')
                f.write('\t\t\t\t<targetName>laterals/' + str(i[0]) + '/water_discharge</targetName>\n')
                f.write('\t\t\t</item>\n')
            ...
            f.write('\t\t<logger>\n')
            f.write('\t\t\t<workingDir>.</workingDir>\n')
            f.write('\t\t\t<outputFile>rr2flow.nc</outputFile>\n')
            f.write('\t\t</logger>\n')
            f.write('\t</coupler>\n')
            f.write('</dimrConfig>\n')
